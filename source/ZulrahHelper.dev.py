...
def switch_to_gear_set(humanizer: UIInteraction, target_gear_set_ids: list[int]):
    ...
    for attempt in range(max_retries):
        # Wait for game state to update. A game tick is 0.6s. A slightly longer, randomized wait is more human-like and reliable.
        time.sleep(0.65 + random.uniform(0.05, 0.15))

        # Get current equipment state from the API
        equipped_after_switch = {item['id'] for item in api.get_equipment() if item['id'] != -1}
        
        # Check if all target items are now equipped
        missing_items = set(target_gear_set_ids) - equipped_after_switch
        
        if not missing_items:
            print(f"[ZulrahHelper] Gear switch successful on attempt {attempt + 1}.")
            # Optional: secondary check for unexpected items for logging purposes
            other_gear_set_ids = MAGIC_GEAR_SET_IDS if target_gear_set_ids == RANGE_GEAR_SET_IDS else RANGE_GEAR_SET_IDS
            unexpected_items = equipped_after_switch.intersection(other_gear_set_ids)
            if unexpected_items:
                print(f"[ZulrahHelper] Note: The following items were unexpectedly equipped but switch is considered complete: {[get_item_display_name(item_id) for item_id in unexpected_items]}")
            
            print(f"[ZulrahHelper] Final equipped gear: {[get_item_display_name(item_id) for item_id in equipped_after_switch]}")
            update_timing_skill()
            enable_user_mouse_input()
            return # Success

        # If check fails, log and prepare for retry
        print(f"[ZulrahHelper] Gear switch check failed on attempt {attempt + 1}.")
        print(f"[ZulrahHelper] Missing items: {[get_item_display_name(item_id) for item_id in missing_items]}")

        if attempt < max_retries - 1:
            print("[ZulrahHelper] Retrying...")
            
            items_to_reclick = set()
            current_inventory_list = api.get_inventory()
            if not current_inventory_list:
                print("[ZulrahHelper] Cannot retry, failed to fetch inventory.")
                continue

            # Find the inventory slots of the items that are missing from the equipment
            for missing_item_id in missing_items:
                found_in_inv = False
                for i, inv_item in enumerate(current_inventory_list):
                    if inv_item['id'] == missing_item_id:
                        items_to_reclick.add(i + 1)
                        found_in_inv = True
                        break
                if not found_in_inv:
                     print(f"[ZulrahHelper] Warning: Missing item {get_item_display_name(missing_item_id)} not found in inventory for retry.")
            
            if not items_to_reclick:
                print("[ZulrahHelper] No missing items found in inventory to re-click. Aborting retries.")
                break

            # Perform the re-clicks
            print(f"[ZulrahHelper] Re-clicking inventory slots: {sorted(list(items_to_reclick))}")
            for slot in sorted(list(items_to_reclick)):
                humanizer.click_inventory_slot(slot)
                # Smaller delay between re-clicks
                time.sleep(random.uniform(SCRIPT['settings']['timings'] * 0.9, SCRIPT['settings']['timings'] * 1.8) * SCRIPT['state']['timing_skill'])

    # If loop finishes without success
    print("[ZulrahHelper] Gear switch FAILED after all retries.")
    final_equipped = {item['id'] for item in api.get_equipment() if item['id'] != -1}
    print(f"[ZulrahHelper] Final (failed) equipped gear: {[get_item_display_name(item_id) for item_id in final_equipped]}")

    update_timing_skill() # Still update skill, as an action was performed
    enable_user_mouse_input()
...
def on_low_health(ui: UIInteraction, stop_event: threading.Event):
    pyautogui.press('f2') # Ensure inventory tab is open
    def consume_food():
        SCRIPT['state']['last_action_time'] = time.time()
