...
class RoutePlanner:
    ...
    def run(self, stop_event: threading.Event):
        """
        Executes the entire route pathing sequence. for route-1
        @TODO make indipendent route methods. so steps can be filtered and other algorithms can be applied like we do right now.
        @ Right now we expect to be in edgeville and want to walk to the grand exchange. and we do actually make it 100% of the time now. so turing complete in my opinion
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        pather = RoutePlanner(route_name=ROUTE_NAME, confidence=CONFIDENCE, overlay=overlay)
        
        # Function to run in the background thread, managed by HotkeyManager
        def pathing_loop_function(hotkey_stop_event): # Accept the argument from HotkeyManager
        ...
        hotkey_manager.register_hotkeys(pathing_loop_function)
        try:
            overlay.root.mainloop()
            hotkey_manager.wait_for_exit()
        except stop_event.is_set() == True | KeyboardInterrupt:
